...
def remove_todo(todo_index: int):
    ...
    try:
        ...
        if 1 <= todo_index <= len(todos):
            ...
            if removed_todo:
                task_name = (
                    removed_todo.get("task", removed_todo)
                    if isinstance(removed_todo, dict)
                    else removed_todo
                )
    ...
    try:
        ...
        for i, todo in enumerate(todo_list):
            if isinstance(todo, dict):
                ...
                item_text = (
                    f"{status_emoji} {i+1}. {task} {priority_emoji.get(priority, '⚡')}"
                )
                if due_date:
    ...
    try:
        ...
        for i, todo in enumerate(todos):
            ...
            if keyword.lower() in task_text.lower():
                status = (
                    "✅" if isinstance(todo, dict) and todo.get("completed") else "⏳"
                )
...
def save_user_name(name: str):
    ...
    try:
        ...
        return (
            f"🙋‍♂️ Thanks, {name}! I'll remember that and personalize our conversations."
        )
    except Exception as e:
...
def count_todos():
    ...
    try:
        ...
        total = len(todos)
        completed = len(
            [t for t in todos if isinstance(t, dict) and t.get("completed", False)]
        )
        pending = total - completed
