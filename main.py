...
def get_ai_response(user_input):
    """Get AI response using OpenRouter API or fallback to intelligent responses"""
    try:
...
def get_intelligent_response(user_input):
    ...
    if any(word in user_lower for word in ['add todo', 'add task', 'todo', 'task']):
        if 'add' in user_lower or 'create' in user_lower:
            ...
            task = user_input
            for phrase in ['add todo:', 'add task:', 'todo:', 'task:', 'add a todo', 'add a task']:
                if phrase in user_lower:
                    task = user_input[user_input.lower().find(phrase) + len(phrase):].strip()
                    break
    ...
    # Mobile/UI specific
    if any(word in user_lower for word in ['mobile', 'responsive', 'ui', 'design', 'beautiful']):
    ...
    # Productivity help
    if any(word in user_lower for word in ['productive', 'productivity', 'organize', 'plan']):
    ...
    if any(word in user_lower for word in ['java', 'teach me java', 'learn java', 'java help']):
...
What specific Java topic would you like to learn more about?"""
    ...
    elif any(word in user_lower for word in ['code', 'programming', 'python', 'javascript', 'html', 'css', 'coding']):
    ...
    if any(word in user_lower for word in ['magical world', 'magic', 'fantasy story', 'story about']):
...
Would you like me to continue this story or help you create your own magical world?"""
    ...
    elif any(word in user_lower for word in ['creative', 'story', 'write', 'writing', 'poem', 'article']):
    ...
    # Analytics request
    if any(word in user_lower for word in ['analytics', 'stats', 'statistics', 'progress']):
        analytics = get_analytics()
...
def update_analytics(action):
    ...
    if action == "task_created":
        ...
        analytics["total_tasks_completed"] = analytics.get("total_tasks_completed", 0) + 1
